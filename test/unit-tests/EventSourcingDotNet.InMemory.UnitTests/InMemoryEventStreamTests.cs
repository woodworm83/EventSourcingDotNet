...
public sealed class InMemoryEventStreamTests
{
    ...
    public async Task ShouldThrowOptimisticConcurrencyException()
    {
        ...
        var actualVersion =
            await eventStore.AppendEventsAsync(aggregateId, new[] { new TestEvent() }, default, default, default);

        await eventStore.Awaiting(x =>
                x.AppendEventsAsync(aggregateId, new[] { new TestEvent() }, new AggregateVersion(42), default, default))
            .Should()
    ...
    public async Task ShouldIgnoreOtherStreamsForAggregateVersion()
    {
        ...
        await eventStore.Awaiting(x =>
                x.AppendEventsAsync(new TestId(), new[] { new TestEvent() }, default, default, default))
            .Should()
            .NotThrowAsync();
    }
}